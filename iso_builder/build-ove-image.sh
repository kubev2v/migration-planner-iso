...
OPTIONS:
    ...
    --registry REGISTRY     Registry URL (required)
    --org ORG               Organization/namespace (required)
...
ENVIRONMENT VARIABLES:
    ...
    ORG                     Organization/namespace (overridden by --org)

EXAMPLES:
    # Basic usage with all required parameters
    $(basename "$0") --iso-file /path/to/rhcos.iso --output-file /path/to/agent.iso --agent-image my-agent --agent-tag latest --registry docker.io --org myorg

    # Specify custom working directory
    $(basename "$0") --iso-file /path/to/rhcos.iso --output-file /path/to/agent.iso --agent-image my-agent --agent-tag latest --registry docker.io --org myorg --dir /tmp/work

    # Dry run to preview operations
    $(basename "$0") --iso-file /path/to/rhcos.iso --output-file /path/to/agent.iso --agent-image my-agent --agent-tag latest --registry docker.io --org myorg --dry-run
...
function setup_vars() {
    ...
    if [[ -z "$REGISTRY" ]]; then
        log_error "REGISTRY is required. Please provide --registry parameter."
        log_error "Use --help for usage information."
        exit 1
    fi
    
    if [[ -z "$ORG" ]]; then
        log_error "ORG is required. Please provide --org parameter."
        log_error "Use --help for usage information."
        exit 1
    fi
...
function setup_agent_artifacts() {
    local image_dir="${work_dir}/images/${AGENT_IMAGE}"
    local pull_spec="${REGISTRY}/${ORG}/${AGENT_IMAGE}:${AGENT_TAG}"
    local image_tar="${image_dir}/${AGENT_IMAGE}.tar"
    ...
    if [[ -f "${image_tar}" ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            log_info "[DRY RUN] Would pull: ${pull_spec}"
            return 0
        fi
